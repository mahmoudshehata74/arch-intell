...
async function generateDesign(modelType, description) {
  try {
    const publicUrl = 'https://i2pdw4623r-496ff2e9c6d22116-5555-colab.googleusercontent.com'; // Replace with actual URL
    const response = await axios.post(`${publicUrl}/generateDesign`, {
      model_type: modelType,
      prompt: description,
    });

    const outputFromFlaskServer = response.data.image;
    console.log('Generated Image:', outputFromFlaskServer);
  } catch (error) {
    console.error('Error generating design:', error);
  }
}

generateDesign('2D', 'Designe my new home living room, kitchen, and bedroom');
